...
Migration: More than just a repository change Migration is a multi-step process • Convert modeled views to Calculation Views within the current repository • Migrate all design time artifacts from the HANA Repository to Git • Convert database artifacts to new HDI syntax and file extensions • Deploy database artifacts via HDI into Containers • Adjust security for HDI container concepts • Adjust consumption for HDI container concept • Convert XS layer artifacts to XSA (Node.js and XSJS compatibility adjustments) • Deploy XS artifacts as micro services
